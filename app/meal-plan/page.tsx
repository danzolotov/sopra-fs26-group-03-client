	...
	return (
		<DashboardShell headerTitle="Meal Plan" selectedMenuKey="5">
				...
				<div className="w-full xl:w-96 flex flex-col gap-6">
					...
					>
						...
						{loading ? (
							...
							<div role="list" className="divide-y divide-slate-50">
								{missingIngredients.map((item) => (
									<div key={item.ingredient.id} role="listitem" className="py-3">
										<div className="flex w-full min-w-0 items-center justify-between gap-3">
											<Text className="min-w-0 break-words text-slate-700 font-medium">
												{item.ingredient.ingredientName}
											</Text>
											<Tag
												color="orange"
												className="m-0 max-w-[45%] shrink-0 whitespace-normal break-words rounded-full border-none bg-orange-50 text-orange-600 font-bold"
											>
												{item.missingQuantity} {item.ingredient.standardUnit.toLowerCase()}
											</Tag>
										</div>
									</div>
								))}
			...
			<Modal
				...
				okText="Schedule Now"
				okButtonProps={{
					className: "bg-slate-900 border-none h-10 rounded-lg px-6 font-semibold",
				}}
