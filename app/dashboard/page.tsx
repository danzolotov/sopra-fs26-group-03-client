...
import { Typography, Card, Button, Spin, Tag, Row, Col } from "antd";
import { CalendarOutlined, RightOutlined, ShoppingCartOutlined } from "@ant-design/icons";
	...
	useEffect(() => {
		...
		const fetchDashboardData = async () => {
			...
			try {
				...
				try {
					const shoppingListData = await apiService.get<ShoppingListGetDTO>(
				...
				try {
					...
					const meals = await apiService.get<MealPlan[]>(
		...
		<DashboardShell headerTitle="Dashboard" selectedMenuKey="1">
			<div className="mb-10 animate-fade-in-up">
				<div className="flex items-center justify-between">
					...
					{error && (
						<Tag color="error" className="rounded-full px-4">
							{error}
						</Tag>
					)}
				</div>
				<Paragraph className="text-slate-500 max-w-2xl text-lg mt-2">
					Here&apos;s what&apos;s happening today in your kitchen. Track your meals and manage your
					pantry with ease.
				</Paragraph>
			</div>
									...
									return (
										...
										>
											<div className="flex items-center gap-5">
												<div
													className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl shadow-sm ${meal ? "bg-white" : "bg-slate-100"}`}
												>
													{getMealIcon(type)}
												</div>
												<div>
													<Text
														className={`block font-bold text-xs uppercase tracking-widest ${meal ? "text-orange-500" : "text-slate-400"}`}
													>
														{type}
													</Text>
													<Text
														className={`text-lg font-bold ${meal ? "text-slate-800" : "text-slate-400"}`}
													>
											...
											{meal && (
												<div className="flex items-center gap-2 px-4 py-2 bg-white rounded-xl shadow-sm border border-orange-50">
													...
													<Text className="text-xs font-bold text-slate-600 uppercase tracking-tight">
														Scheduled
													</Text>
												</div>
											)}
							...
							<div className="space-y-3">
								{(shoppingList?.items ?? shoppingList?.shoppingListItems ?? [])
									...
									.map((item) => (
										<div
											key={item.id}
											className="flex justify-between w-full items-center px-1 py-4 border-slate-50"
										>
								...
								{(shoppingList?.items ?? shoppingList?.shoppingListItems ?? []).filter(
									(item) => !item.isBought,
								).length === 0 ? (
