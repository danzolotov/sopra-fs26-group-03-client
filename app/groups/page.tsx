...
const GroupsPage: React.FC = () => {
	...
	const handleCreateGroup = async (values: CreateGroupFormValues) => {
		...
		try {
			...
			router.push(`/groups/me?action=created&groupName=${encodeURIComponent(createdName)}`);
		} catch (error) {
				...
				<div className="flex flex-1 items-center justify-center px-4 py-8">
					<Card className="w-full max-w-md rounded-[2rem] border border-primary-500/20 bg-white/90 p-8 text-center shadow-xl backdrop-blur">
						...
						<p className="mb-8 text-secondary-600">
							...
							<span className="font-bold text-slate-900">{existingGroup.name}</span>. Leave your
							current group if you want to join or create a new one.
						</p>
						<Button
